...
class Difference {
  ...
  hideItems(items) {
    ...
    //(i !== arr.length - 1) - the element we iterate over is not the last in the array
  }
...
class Form {
  ...
  init() {
    ...
    this.initMask();
    Array.from(this.forms).forEach(item => {
...
class VideoPlayer {
    ...
    this.btns.forEach((btn, i) => {
      ...
      btn.addEventListener("click", () => {
        if (!btn.closest(".module__video-item") || btn.closest(".module__video-item").getAttribute("data-disabled") !== "true") {
          // if the block is not locked
          this.activeBtn = btn;
          if (document.querySelector("iframe#frame")) {
            this.overlay.style.display = "flex";
            if (this.path !== btn.getAttribute("data-url")) {
              this.path = btn.getAttribute("data-url");
              // method loadVideoById - the documentation YouTube Player
  ...
  onPlayerStateChange(state) {
    try {
      ...
      //cloneNode(true) - deep copy so that all properties are copied

      //state.data === 0 - when the user has fully watched the video
      if (state.data === 0) {
...
class MainSlider extends _slider__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  showSlides(n) {
    ...
    //subtract 1 because the array starts with 0
